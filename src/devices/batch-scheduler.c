...
static struct condition bus_available;

static int waiting_pri_send, waiting_pri_recv;
static int waiting_pri;
static int slot_used, current_direction;
static bool bus_in_use;
...
void init_bus (void) {
  ...
  // Initi lock and condition vars
  lock_init(&bus_lock);
  cond_init(&bus_available);
...
// helper func to check the bus rules
bool check_bus (const task_t *task) {
  //rule no.1: slots used < 3
  if (slot_used >= 3){
    return false;
  }
  // rule no.2: if bus is in use, the task direciton should = current bus direction 
  else if (bus_in_use & task->direction != current_direction){
    return false;
  }
  //rule no.3: if normal task, can not proceed if a priority task in slot
...
void get_slot (const task_t *task) {
  ...
  while (!check_bus(task)) {
    cond_wait (&bus_available, &bus_lock);
  }
...
void release_slot (const task_t *task) {
  ...
  cond_broadcast(&bus_available, &bus_lock);
